...
const settings={...defaults,...animationDefaults,seed:1427,palette:0,x:.96,y:.5,background:'#080e25',format:'wide'};
...
uniform vec2 res,focus;uniform float seed,width,pixel,stretch,chaos,glow,grain,angle,time,waveLength,colorWave;uniform vec3 bg,c1,c2,c3;
float hash(vec2 p){return fract(sin(dot(p,vec2(127.1,311.7))+seed*12.37)*43758.5453);}
void main(){vec2 uv=gl_FragCoord.xy/res;vec2 q=(uv-focus)*vec2(res.x/res.y,1.);float a=angle*3.14159265/180.;q=mat2(cos(a),-sin(a),sin(a),cos(a))*q;float px=pixel/900.;vec2 cell=vec2(px*(1.+stretch*.09),px);vec2 id=floor(q/cell);vec2 p=(id+.5)*cell;
...
$('reset').onclick=()=>{Object.assign(settings,defaults,animationDefaults,{seed:1427,palette:0,x:.96,y:.5,background:'#080e25',format:'wide'});time=0;sync();render()};
...
function move(e){const r=canvas.getBoundingClientRect();settings.x=Math.max(0,Math.min(1,(e.clientX-r.left)/r.width));settings.y=Math.max(0,Math.min(1,(e.clientY-r.top)/r.height));render()}
